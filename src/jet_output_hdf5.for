...
!***********************************************************************
      Subroutine readHydroInfoYingru(t,x,y,z,e,s,Temp,vx,vy,vz,ctl)
! mimic 3D hydro behavior
! -- t,x,y,z: input
! -- e,s,Temp,(vx,vy,vz): output
! -- ctl: 0-read successfully; 1-x/y out of range; 2- tau>maxTau;
!         3-tau<tau0
    
      Implicit None
      Double precision:: t,x,y,z,e,s,Temp,vx,vy,vz,p
      Integer:: ctl
     ...
     &                       hydroGrid_numOfframes
      ...
      ctl=0
      tau = sqrt(t*t-z*z)
      If (tau < hydroGrid_Tau0) then
        ctl=3
        Return
      endif

      If (tau > hydroGrid_Taumax) then
        ctl=2
        Return
      endif

      max_x = hydroGrid_XH*hydroGrid_DX
      max_y = hydroGrid_YH*hydroGrid_DY
      if (abs(x) > max_x .or. abs(y)>max_y) then
        ctl = 1
        Return
      endif
...
!     &        tau, hydroGrid_Tau0, hydroGrid_dTau, tauI, tauInc
